-- Config for the volunteer email relay (Google Apps Script web app).
-- Single-row table read only by the volunteer-register edge function
...
-- See supabase/functions/volunteer-register/apps-script.gs.
--
-- Applied to project ldxpockcgcxvsrbyhcnt. The shared_secret value is set
-- directly in the database, never committed.

create table if not exists public.volunteer_email_relay_config (
...
alter table public.volunteer_email_relay_config enable row level security;
-- No policies: service-role access only (edge function).
